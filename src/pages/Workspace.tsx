...
const INITIAL_CAMPAIGNS: CampaignItem[] = [
  {
    id: '1',
    creator: '@shubham_make',
    videoTitle: 'Utility Streetwear Lookbook',
    productSKU: 'JKT-WRK-01',
    progress: 80,
    conversion: '4.8%',
    status: 'active',
  },
  {
    id: '2',
    creator: '@alex_fit',
    videoTitle: 'Urban Canvas Fit Guide',
    productSKU: 'JKT-WRK-01',
    progress: 40,
    conversion: '2.1%',
    status: 'active',
  ...
  {
    id: '3',
    creator: '@sarah_styles',
    videoTitle: 'Minimalist Autumn Outerwear',
    productSKU: 'BAG-UTL-02',
    progress: 100,
    conversion: '7.5%',
    status: 'review',
  },
  {
    id: '4',
    creator: '@dev_designer',
    videoTitle: 'Core Utility Capsule Collection',
    productSKU: 'CAP-STR-03',
    progress: 10,
    conversion: '0.0%',
    status: 'paused',
  },
...
const INITIAL_TASKS: TaskItem[] = [
  {
    id: 't1',
    text: 'Map Canvas Worker Jacket to Creator @shubham_make summer reel',
    completed: false,
    priority: 'high',
  },
  {
    id: 't2',
    text: 'Audit commission split settings for creator @alex_fit',
    completed: true,
    priority: 'medium',
  },
  {
    id: 't3',
    text: 'Update pricing records for Bag Utility series in shop database',
    completed: false,
    priority: 'low',
  },
  {
    id: 't4',
    text: 'Verify Webhook events receiver state on checkout route',
    completed: false,
    priority: 'high',
  },
]
...
export const Workspace: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = React.useState<'overview' | 'campaigns' | 'products'>(
    'overview'
  )
  ...
  const handleToggleTask = (id: string) => {
    ...
    if (task) {
      addLog(
        `TASK STATUS: "${task.text.substring(0, 30)}..." updated to ${!task.completed ? 'COMPLETED' : 'ACTIVE'}`
      )
    }
  }
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-4 border-b border-border">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Manage creator video checkout integrations, active product mapping, and split telemetry
            logs.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card hoverEffect={false}>
          ...
          <CardContent className="p-6 pt-0">
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              Platform aggregate checks since last deployment
            </p>
          </CardContent>
        </Card>
        ...
        <Card hoverEffect={false}>
          ...
          <CardContent className="p-6 pt-0">
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              28 verified live checkouts active now
            </p>
          </CardContent>
        </Card>
        ...
        <Card hoverEffect={false}>
          ...
          <CardContent className="p-6 pt-0">
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              Split escrow node balances cleared
            </p>
          </CardContent>
        </Card>
        ...
        >
          ...
          <CardContent className="p-6 pt-0">
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              Requires manual catalog verification
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <CardHeader className="border-b border-border pb-4 flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription className="text-xs mt-1">
                    Current status of creator shop integrations
                  </CardDescription>
                </div>
          ...
          {activeTab === 'campaigns' && (
            <Card hoverEffect={false}>
              <CardHeader className="border-b border-border">
                ...
                <CardDescription>
                  Comprehensive creator listing and split ratios configuration
                </CardDescription>
              </CardHeader>
              <CardContent className="p-6 space-y-4">
                <div className="space-y-4">
                  {campaigns.map((c) => (
                    <div
                      key={c.id}
                      className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-border rounded-lg bg-card/50 hover:bg-muted/10 transition-all gap-4"
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          {c.status === 'active' && (
                            <Badge
                              variant="default"
                              className="bg-emerald-600 hover:bg-emerald-600/90 text-white text-[9px] px-1.5 py-0.2 rounded-full"
                            >
                              Active
                            </Badge>
                          )}
                        ...
                        <div className="text-[11px] font-mono text-muted-foreground">
                          Product Link: {c.productSKU} | Conversion Ratio: {c.conversion}
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {PRODUCTS.map((prod) => (
                    <Card
                      key={prod.sku}
                      hoverEffect={true}
                      className="p-4 space-y-3 bg-card border-border shadow-none"
                    >
            ...
            <Card hoverEffect={false}>
              <CardHeader className="border-b border-border pb-4">
                ...
                <CardDescription className="text-xs">
                  Active products inventory and link counts
                </CardDescription>
              </CardHeader>
              <CardContent className="p-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {PRODUCTS.map((prod) => (
                    <div
                      key={prod.sku}
                      className="border border-border p-4 rounded-lg bg-card text-foreground shadow-sm space-y-2"
                    >
                      <div className="flex justify-between items-center">
                        ...
                        <span
                          className={`w-2 h-2 rounded-full ${prod.status === 'In Stock' ? 'bg-emerald-500' : 'bg-destructive'} inline-block`}
                        ></span>
                      </div>
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="space-y-2.5">
                {tasks.map((task) => (
                  ...
                  >
                    <div
                      className={`mt-0.5 w-4 h-4 border rounded flex items-center justify-center transition-colors ${task.completed ? 'bg-primary border-primary text-primary-foreground' : 'border-input bg-background'}`}
                    >
                      {task.completed && <Check className="w-3 h-3" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p
                        className={`text-xs leading-snug font-medium ${task.completed ? 'line-through text-muted-foreground' : 'text-foreground'}`}
                      >
                        {task.text}
                      </p>
                      <div className="flex items-center gap-1.5 mt-1.5 text-[10px] text-muted-foreground">
                        <Badge
                          variant="outline"
                          className="text-[8px] py-0 px-1 font-bold uppercase"
                        >
                          {task.priority}
                        </Badge>
                        <span>•</span>
          ...
          <Card
            hoverEffect={false}
            className="border-border rounded-lg bg-zinc-950 dark:bg-zinc-950/80 text-zinc-400 p-4 font-mono text-[10px] leading-relaxed h-72 flex flex-col justify-between shadow-sm"
          >
